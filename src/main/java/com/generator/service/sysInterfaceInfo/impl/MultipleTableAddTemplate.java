...
public class MultipleTableAddTemplate {

    private static String createFilePath(String tableName) {
        String path = System.getProperty("user.dir");
        String filePath = path + File.separator + tableName + "_tables_add" + ".xlsx";
        return filePath;
    }
    ...
    private static void createSheet1TemplateHeader(String tableName, List<ExcelCell> cells) throws Exception {
        ...
        //第二列
        String[] secondRow = {tableName, "", UnderlineToCamelUtils.underlineToCamel(tableName, true), "addBeans", "add"};
    ...
    private static void createSheet1TemplateContent(List<Map<String, Object>> mainTableColumnList, List<List<Map<String, Object>>> subTableList, List<ExcelCell> cells) throws Exception {
        ...
        row += mainTableColumnList.size()-1;
    ...
    private static void createSheet2SubTable(int row, String tableName, String[] subTables, List<List<Map<String, Object>>> subTableList, List<ExcelCell> cells) throws Exception {
        for(int i = 0; i < subTableList.size(); i++){
            ...
            for(Map<String, Object> tableColumn : subTable){
                if(BaseUtil.returnString(tableColumn.get("columnName")).equals("id")){
                    continue;
                }
                //条件序号
                ...
                sqlBatchId.setRowNum(row);
                sqlBatchId.setColNum(0);
                sqlBatchId.setCellValue(BaseUtil.returnString(i+2));
                cells.add(sqlBatchId);
                ...
                tableCode.setRowNum(row);
                tableCode.setColNum(2);
                tableCode.setCellValue(subTables[i]);
                cells.add(tableCode);
                ...
                // 字段别名
                String paramValue = UnderlineToCamelUtils.underlineToCamel(BaseUtil.returnString(tableColumn.get("columnName")), true);
                String mainTableId = UnderlineToCamelUtils.underlineToCamel(tableName, true) + "Id";
                if(paramValue.equals(mainTableId)){
                    paramValue = "sql_1_id";
                }
                ExcelCell paramCode = new ExcelCell();
                paramCode.setRowNum(row);
                paramCode.setColNum(4);
                paramCode.setCellValue(paramValue);
                cells.add(paramCode);

                // 父级节点
                ExcelCell fatherNodeCode = new ExcelCell();
                fatherNodeCode.setRowNum(row);
                fatherNodeCode.setColNum(5);
                String suffix = "";
                if(i > 0){
                    suffix = BaseUtil.returnString(i);
                }
